    ...
    <div>
      ...
      {layoutKey === 'cover' && content && setContent && (
        <div style={{ marginBottom: 22, padding: '14px 14px 4px', background: 'rgba(192,57,43,0.06)', border: '1px solid rgba(192,57,43,0.35)' }}>
          ...
          <Field label="IG caption" help={window.BRAND.captionHelp}>
            <FieldInput
...
// PROJECT_INSTRUCTIONS and TOPIC_PROMPT live in the brand pack
// (shared/brands/<slug>/prompts.js) as window.BRAND.projectInstructions /
// window.BRAND.topicPrompt — each brand ships its own Claude-Project prompts.
function PasteTab({ content, setContent, setCurrentIdx }) {
  ...
  const copyProjectInstructions = async () => {
    try {
      await navigator.clipboard.writeText(window.BRAND.projectInstructions);
  ...
  const copyTopic = async () => {
    try {
      await navigator.clipboard.writeText(window.BRAND.topicPrompt);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
        ...
        <a href={`/brands/${window.BRAND.slug}/examples.yaml`} download="examples.yaml" title="One-time setup: upload as a Knowledge file in your Claude Project." style={{
          flex: 1, background: '#1a1a1a', color: '#E8DCC8', border: '1px solid #2c2c2c',
          padding: '10px 12px', cursor: 'pointer',
          font: '10px "DM Sans", system-ui', letterSpacing: 1.6, textTransform: 'uppercase', fontWeight: 600,
      ...
      <details style={{ marginTop: 6 }}>
        ...
        <div style={{ fontSize: 12, color: '#bbb', lineHeight: 1.7, marginTop: 8 }}>
          <p style={{ marginTop: 0 }}>Set up one Claude Project with the {window.BRAND.name} voice baked into custom instructions and your source books/PDFs uploaded as project knowledge. After that, every new carousel is one chat message → paste here.</p>

          <b style={{ color: '#E8DCC8' }}>One time — set up the Claude Project:</b>
          <ol style={{ paddingLeft: 18, marginTop: 4, marginBottom: 12 }}>
            <li>Open <a href="https://claude.ai" target="_blank" style={{ color: '#C0392B' }}>claude.ai</a> → <b>Projects</b> → <b>Create project</b> (name it e.g. "{window.BRAND.suggestedProjectName}").</li>
          ...
          <ol style={{ paddingLeft: 18, marginTop: 4, marginBottom: 12 }}>
            <li>Open your {window.BRAND.name} project, click <b>New chat</b>.</li>
